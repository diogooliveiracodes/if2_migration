...
$router->post('/properties', function () use ($router){
    ...
    $consulta = $pdo->query("SELECT p.id, p.reference, p.status, p.situation, p.exclusive, p.position, p.zone,
        p.constructed_year, p.reform_year, p.solar_orientation, p.schedule_visit, p.financing, p.exchange_accept, p.bedroom, p.suite, p.bathroom,
        p.kitchen, p.vacancy, p.housemaidroom, p.room, p.hobby_box, p.currency, p.main_purpose, p.hide_price, p.valued_sale,
        p.commission_broker, p.valued_rent, p.valued_season, p.exchange_property_value, p.iptu_price, p.iptu_period,
        parcels, p.condo_price, p.usefull_area_measure, p.constructed_area_measure, p.private_area_measure,
        p.common_area_measure, p.terrain_area_measure, p.total_area_measure, p.condo_id, p.zipcode,
        p.country, p.estate, p.city_id, p.neighborhood_id, p.street, p.number, p.complement, p.reference,
